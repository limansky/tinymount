...
namespace
{
    Tinymount::ErrorCode mapErrorName(const QDBusError& error)
    {
        Tinymount::ErrorCode err = Tinymount::OK;
...
DiskManagerUDisks1::DiskManagerUDisks1(DiskManagerImplListener &listener, QObject *parent)
    : QObject(parent), listener(listener), ready(false)
{
...
bool DiskManagerUDisks1::isReady() const
{
    return ready;
}
...
void DiskManagerUDisks1::unmountDevice(const QString& path, bool force)
{
...
void DiskManagerUDisks1::detachDevice(const QString &path)
{
...
void DiskManagerUDisks1::onMountComplete(QDBusPendingCallWatcher* call)
{
    ...
    DeviceMap::const_iterator it = deviceCache.find(path);
    ...
    listener.onDeviceMounted(**it, mountPath, mapErrorName(r.error()));
...
void DiskManagerUDisks1::onUnmountComplete(QDBusPendingCallWatcher* call)
{
    ...
    listener.onDeviceUnmounted(**it, mapErrorName(r.error()));

    call->deleteLater();
}

DeviceInfoPtr DiskManagerUDisks1::deviceForPath(const QDBusObjectPath &path)
{
